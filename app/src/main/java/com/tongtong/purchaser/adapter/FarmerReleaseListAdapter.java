...
public class FarmerReleaseListAdapter extends BaseAdapter {

	private List<FarmerReleaseInformationModel> items;
	private Context mContext;

	public FarmerReleaseListAdapter(Context c,
	...
	public View getView(int position, View view, ViewGroup arg2) {
		view = LayoutInflater.from(mContext).inflate(
				R.layout.farmer_release_list_item, null);
		...
		title.setText((items.get(position).getFarmer()!=null?items.get(position).getFarmer().getName():"") + "("
		...
		httpImageLoadTask
				.execute(items.get(position).getFarmer()!=null?items.get(position).getFarmer().getHeadUrl():"");
		return view;
	}
