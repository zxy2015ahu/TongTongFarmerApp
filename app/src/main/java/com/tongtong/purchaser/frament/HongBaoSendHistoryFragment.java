...
import com.tongtong.purchaser.adapter.HongBaoSendAdapter;
...
public class HongBaoSendHistoryFragment extends BaseFrament implements View.OnClickListener,HttpTask.HttpTaskHandler,
    ...
    private HongBaoSendAdapter adapter;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter=new HongBaoSendAdapter(getActivity());
    ...
    public void taskSuccessful(String str, int code) {
        ...
        if(verification(selectResultCode)){
            if(adapter.getHeaderCount()==0){
                adapter.addHeader(new RecyclerArrayAdapter.ItemView() {
                    ...
                    public View onCreateView(ViewGroup parent) {
                        View header=View.inflate(getActivity(),R.layout.rp_send_record_list_header,null);
                        final ImageView iv_avatar=(ImageView) header.findViewById(R.id.iv_avatar);
                        Glide.with(getActivity()).load(NetUtil.getFullUrl(selectResultJson.get("headUrl").getAsString()))
                        ...
                        ((TextView)header.findViewById(R.id.tv_send_money_amount)).setText("￥"+selectResultJson.get("total_amount").getAsDouble());
                        ((TextView)header.findViewById(R.id.tv_send_money_count)).setText(Html.fromHtml("发出红包<font color=\"#F44336\">"+selectResultJson.get("total_size").getAsInt()+"</font>个"));
                        return header;
                    }
            ...
            for(int i=0;i<qiang_items.size();i++){
                ...
                HongbaoSendModel hm=new HongbaoSendModel();
                hm.setAdd_time(item.get("add_time").getAsString());
                hm.setId(item.get("id").getAsInt());
                hm.setStatus(item.get("status").getAsInt());
                hm.setTotal_amount(item.get("total_amount").getAsDouble());
                hm.setTotal_num(item.get("total_num").getAsInt());
                hm.setType(item.get("type").getAsInt());
                hm.setTypeid(item.get("typeid").getAsInt());
                hm.setYiling(item.get("yiling").getAsInt());
                adapter.add(hm);
            }
            if(start_page==0){
    ...
    private void loadData(){
        ...
        task.execute(UrlUtil.GET_RP_SEND_LIST,dataJson.toString());
    }
    ...
    public void onItemClick(int position) {
        HongbaoSendModel model=adapter.getItem(position);
        opendetails(model.getId());
    }
