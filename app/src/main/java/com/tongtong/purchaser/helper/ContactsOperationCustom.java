...
public class ContactsOperationCustom extends IMContactsOperation {
    ...
    private String TAG=ContactsOperationCustom.class.getSimpleName();
    ...
    public boolean onListItemClick(Fragment fragment, IYWContact contact) {
        if (contact.getAppKey().equals(YWConstants.YWSDKAppKey)) {
            ...
            intent.putExtra(ChattingActivity.TARGET_ESERVICE, Constant.SERVER_ACCOUNT);
            fragment.startActivity(intent);
        } else {
            Intent intent=new Intent();
            intent.setClass(fragment.getActivity(),ChattingActivity.class);
            intent.putExtra(ChattingActivity.TARGET_ID,contact.getUserId());
            intent.putExtra(ChattingActivity.TARGET_APP_KEY,contact.getAppKey());
            fragment.startActivity(intent);
        }
        return true;
    }
    ...
    public boolean onListItemLongClick(Fragment fragment, final IYWContact contact) {
        ...
        final IYWContactService contactService = UserUtil.getIMKitInstance(fragment.getActivity()).getContactService();
