 ...
 * @author shuheng
 */
public class MYWSDKGlobalConfig extends YWSDKGlobalConfig {


    private MYWSDKGlobalConfig(Pointcut pointcut) {
        super(pointcut);
    }

    //用于给主程序层拿配置
    public volatile static MYWSDKGlobalConfig mInstance =null;

    public static MYWSDKGlobalConfig getInstance(){
        if (mInstance == null) {
            synchronized(MYWSDKGlobalConfig.class) {
                if (mInstance == null) {
                    mInstance = new MYWSDKGlobalConfig(null);
                }
            }
        }
        return mInstance;
